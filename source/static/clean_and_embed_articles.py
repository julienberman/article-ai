...
def clean(df_real, df_sim):
    ...
    df = pd.concat([df_real, df_sim])  # Fix: concat takes a list
    df_clean = (
        df
        .assign(text=lambda x: x["text"].str.lower())
        .assign(date=lambda x: x["date"])
    )
    return df_clean  # Fix: was returning df instead of df_clean
...
def embed(df, model="text-embedding-3-large", batch_size=64):
    ...
    for batch_idx in range(n_batches):
        ...
        batch_ids = batch_df["article_id"].tolist()
        ...
        for article_id, emb_obj in zip(batch_ids, response.data):
            data.append({
    ...
    df_with_embeddings = df.merge(df_embeddings, on="article_id")
